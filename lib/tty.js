...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on('create', function(cols, rows, userdata, func) {
    return session.handleCreate(cols, rows, userdata, func);
  });
...
Session.prototype.handleCreate = function(cols, rows, userdata, func) {
  var self = this
    , terms = this.terms
  ...
  var shell = typeof conf.shell === 'function'
    ? conf.shell(this, userdata)
    : conf.shell;

  var shellArgs = typeof conf.shellArgs === 'function'
    ? conf.shellArgs(this, userdata)
    : conf.shellArgs;
